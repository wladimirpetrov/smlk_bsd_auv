...
// C/C++ source code generated on : Fri Jun 30 11:16:48 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ctrl_TT_ad_ctrl_cpp_ros\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ctrl_TT_ad_ctrl_cpp_ros_M, (NULL));
        ctrl_TT_ad_ctrl_cpp_ros_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ctrl_TT_ad_ctrl_cpp_ros_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ctrl_TT_ad_ctrl_cpp_ros_M)
                == (NULL)) || rtmGetStopRequested(ctrl_TT_ad_ctrl_cpp_ros_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        ctrl_TT_ad_ctrl_cpp_ros_terminate();
        mNode.reset();
      }
    }
    ...
    {
      mRunModel = (rtmGetErrorStatus(ctrl_TT_ad_ctrl_cpp_ros_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
